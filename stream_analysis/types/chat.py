...
class StrReplaced(str):
    _replacement = {
        r':_RipP:': 'P',
        r':skull:7': 'o7',
        r'(:_deadbeats:|:face-red-heart-shape:|:red_heart:)': '❤️',
        r':_sleepy:': '😴',
        r'\bikz+z*\b': 'let\'s go!!!',
        r':_hearteyes:': '😍',
        r':_canteloupe:': '🍈',
    ...
    def __new__(cls, s: str, emotes: list = []):
        _s = s
        for emote in emotes:
            if emoji_pattern.search(emote.get('id', '')):
                _s = re.sub(emote.get('name'), emote.get('id'), _s)
...
class ChatItem(dict):
    ...
    def __init__(self, chat_item: dict, env_: Env_, *args, **kwargs) -> None:
        data = {
            'message_id': chat_item.get('message_id'),
            'message': chat_item.get('message'),
            'transformed_message': StrReplaced(chat_item.get('message', ''), chat_item.get('emotes', [])),
            'message_type': chat_item.get('message_type'),
            'money': chat_item.get('amount'),
            'sub_message': chat_item.get('sub_message'),
            'action_type': chat_item.get('action_type'),
            'action': chat_item.get('action'),
            'icon': chat_item.get('icon'),
            'target_message_id': chat_item.get('target_message_id'),
            'viewer_is_creator': chat_item.get('viewer_is_creator'),
            'ticker_duration': chat_item.get('ticker_duration'),
            'time_text': chat_item.get('time_text'),
            'time_in_seconds': chat_item.get('time_in_seconds'),
            'timestamp': chat_item.get('timestamp'),
        }
        data['neutral'], data['negative'], data['positive'], data['sentiment'], = get_sentiment(
